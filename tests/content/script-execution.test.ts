...
describe('Script Execution Security', () => {
  ...
  it('should block chrome API access', async () => {
    await expect(handleExecuteScript({ script: 'chrome.runtime' }))
      .rejects.toThrow('脚本包含禁止的操作');
  });

  it('should block fetch calls', async () => {
    await expect(handleExecuteScript({ script: 'fetch("https://evil.com")' }))
      .rejects.toThrow('脚本包含禁止的操作');
  });

  it('should block localStorage access', async () => {
    await expect(handleExecuteScript({ script: 'localStorage.getItem("key")' }))
      .rejects.toThrow('脚本包含禁止的操作');
  });

  it('should block nested eval', async () => {
    await expect(handleExecuteScript({ script: 'eval("test")' }))
      .rejects.toThrow('脚本包含禁止的操作');
  });

  it('should block nested Function', async () => {
    await expect(handleExecuteScript({ script: 'Function("return 1")' }))
      .rejects.toThrow('脚本包含禁止的操作');
  });

  it('should handle syntax errors gracefully', async () => {
    await expect(handleExecuteScript({ script: 'invalid syntax here' }))
      .rejects.toThrow('脚本执行错误');
  });
